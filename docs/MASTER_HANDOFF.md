...
1. `feat/live-dinov2` — make DINOv2 inference run on newly uploaded plates and feed the fused/pair heads.
2. `feat/product-ui` — turn the current technical dashboard into a polished examination workflow without altering model semantics.
3. `feat/oracle-deploy` — production/staging Docker deployment, PostgreSQL and durable object storage on Oracle.
4. `feat/qa-hardening` — integration tests, bad-input tests, non-clinical guardrails, regression suite and demo smoke script.
