...
def build(root: Path, *, size: int = 64) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    ...
    both_sides = (
        records.groupby(["subject_id", "label"])["side"]
        .agg(lambda values: {str(v) for v in values})
        .reset_index()
    )
    both_sides["both_sides"] = both_sides["side"].map(lambda sides: {"LEFT", "RIGHT"}.issubset(sides))
    both_counts = both_sides.groupby("label")["both_sides"].sum().to_dict()
    summary = {
        "dataset": "DMR-IR",
        ...
        "both_sides_by_label": {str(key): int(value) for key, value in both_counts.items()},
